...
function Calendar({
  ...
  formatters,
  components,
  eventDates = [], // Array<{ date: Date; count: number }> or Date[]
  ...props
}: any) {
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "group/calendar border border-border/60 bg-background p-2 text-foreground dark:border-white/15 [--cell-radius:var(--radius-md)] [--cell-size:--spacing(7)] in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent",
      ...
      components={{
        ...
        // Pass eventDates down into DayButton via a closure
        DayButton: ({ ...props }) => (
          <CalendarDayButton locale={locale} eventDates={eventDates} {...props} />
        ),
...
function CalendarDayButton({ className, day, modifiers, locale, eventDates = [], ...props }: any) {
  const defaultClassNames = getDefaultClassNames()
  ...
  const hasEvent = React.useMemo(() => {
    ...
    return eventDates.some((item: any) => {
      // Support both plain Date and { date: Date; count: number }
      const eventDate = item instanceof Date ? item : item?.date
  ...
  return (
    ...
    >
      ...
      {/* Red dot indicator — shown below the date number when events exist */}
      {hasEvent && (
        <span
          aria-hidden="true"
